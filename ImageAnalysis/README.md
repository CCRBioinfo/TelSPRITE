...
Additionally, for each nucleus the script saves a QC image with segmentations of each channel, as well as an image containing histograms of both randomized pixel count and focus count overlap, with a red line marking the observed overlap.
...
```
usage: segment_all_channels.py [-h] -i INDIR -o OUTDIR [--min MIN] [--max MAX]
                               [--dapi DAPI] [--debug] [-t] [-b]
